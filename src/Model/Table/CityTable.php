...
class CityTable extends Table
{
    ...
    public function initialize(array $config)
    {
        ...
        $this->table('city');
        $this->displayField('cityname');
        $this->primaryKey('cityid');
    ...
    public function validationDefault(Validator $validator)
    {
        $validator
            ->integer('cityid')
            ->allowEmpty('cityid', 'create');

        $validator
            ->requirePresence('cityname', 'create')
            ->notEmpty('cityname')
            ->add('cityname', 'unique', ['rule' => 'validateUnique', 'provider' => 'table']);
    ...
    {
        $rules->add($rules->isUnique(['cityname']));
